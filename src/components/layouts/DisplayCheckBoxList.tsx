...
export default function DisplayCheckboxRoomList({
  ...
  submitBuildingName,
    ...
    const [checkboxState, setCheckboxState] = useState<{ [key: string]: boolean }>(BuildingNameObject);
    const trueKeys = Object.keys(checkboxState).filter(key => checkboxState[key] === true);
    ...
    const handleCheckBox = (room: string) => {
       ...
       const updatedKeys:string[] = checkboxState[room] ? checkboxStateKey.filter((key) => key !== room) : [...checkboxStateKey, room];
      ...
      <>
        <div
          ...
          style={{
            ...
            clear:parseFloat(roomNumbers) % 10 === 1 && index !== 0 ? "left" : undefined
          }}
        >
          <CheckBoxRooms
